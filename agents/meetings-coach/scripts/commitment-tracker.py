#!/usr/bin/env python3
"""
commitment-tracker.py — Read and filter commitments from the shared brain.

Reads commitments/active.md, filters for source_agent=meetings-coach,
and reports on open, overdue, and approaching items.

Usage:
  python3 commitment-tracker.py                    # All open items
  python3 commitment-tracker.py --person SLUG      # Filter by person
  python3 commitment-tracker.py --overdue-only     # Only overdue items
...
def main():
    person, overdue_only = parse_args()

    commitments = parse_commitments(BRAIN_COMMITMENTS)

    # Filter for meetings-coach source
    commitments = [c for c in commitments if c.get("source_agent") == "meetings-coach"]
        ...
        if by_when:
            try:
                ...
                if delta < 0:
                    ...
                    # Update status if still marked open
                    c["status"] = "overdue"
                elif delta <= 2:
                    is_approaching = True
