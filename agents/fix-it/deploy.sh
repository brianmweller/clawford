...
oc cron add \
  ...
  --no-deliver \
  --failure-alert --failure-alert-to "$TELEGRAM_CHAT_ID" --failure-alert-account-id "$TELEGRAM_ACCOUNT" --failure-alert-channel telegram \
  --message "Check crons for ALL agents and re-register any missing ones. 1) Run: openclaw cron list. 2) Read ~/.openclaw/fix-it-workspace/expected-crons.json — this contains the full cron config for every agent: name, schedule, delivery mode (no_deliver/announce), account, to, AND the complete message prompt. 3) For each agent in the config, check that every expected cron name appears in the live cron list. 4) If ALL present: produce NO output. 5) If any are missing: re-register using the FULL config from the JSON — openclaw cron add --agent {agent} --name {name} --cron {schedule} --to {to} --account {account} {--no-deliver if no_deliver else --announce} --message {the full message from the JSON}. This restores the cron with its complete original prompt, not a placeholder. 6) After re-registering, send a Telegram alert: '🦊🔧 Cron repair: re-registered {list of agent:cron pairs}'. DO NOT touch fix-it.status.md."
