...
oc cron add \
  ...
  --no-deliver \
  --failure-alert --failure-alert-to "$TELEGRAM_CHAT_ID" --failure-alert-account-id "$TELEGRAM_ACCOUNT" --failure-alert-channel telegram \
  --message "Check crons for ALL agents and re-register any missing ones. 1) Run: openclaw cron list. 2) Read ~/.openclaw/fix-it-workspace/expected-crons.json — this lists every expected cron per agent with its schedule and delivery mode. 3) For each agent, check that every expected cron name appears in the cron list. 4) If ALL crons present: produce NO output. 5) If any are missing: re-register them using openclaw cron add --agent {agent} --name {name} --cron {schedule} --to 111111111 --account default {--no-deliver or --announce per the config}. The message for re-registered crons can be a simple placeholder: 'Re-registered by Mr Fixit cron-self-check. Original prompt in deploy.sh.' 6) After re-registering, send a Telegram alert: '🦊🔧 Cron repair: re-registered {list of agent:cron pairs}'. DO NOT touch fix-it.status.md."
