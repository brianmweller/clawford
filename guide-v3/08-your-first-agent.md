...
*Last updated: 2026-04-20 · Reading time: ~10 min · Difficulty: moderate*
...
- Deploy in order: **Mr Fixit → Lowly Worm → Mistress Mouse → Sergeant Murphy → Huckle Cat.** Infra first, then the low-stakes Google-OAuth family, then the relationship agent that benefits from the rest of the fleet already writing to the shared brain. Each new deploy inherits the monitoring and test harness of the ones before it.
...
- Every agent has eight workspace files — five identity / conversational docs (`SOUL.md`, `IDENTITY.md`, `USER.md`, `AGENTS.md`, `MEMORY.md`) and three operational docs (`TOOLS.md`, `HEARTBEAT.md`, `CRONS.md`). The identity five moved to `~/Dropbox/openclaw-backup/agents/<agent>/` in the 2026-04 brain migration — live-synced to the laptop via Dropbox, redundant to the cloud, readable by the inbox dispatcher at every message. The operational three stay in the workspace (`~/.clawford/<agent>-workspace/`), because their contents are per-install rather than per-operator. SOUL/IDENTITY/AGENTS/TOOLS get `chattr +i`-locked on the VPS after deploy; MEMORY, USER, HEARTBEAT, and CRONS stay writable. [Ch 07](07-intro-to-agents.md#the-eight-workspace-files) has the full eight-file table.
...
2. **Bootstrap and personalize the agent's config files.** The five identity docs live in the Dropbox brain at `~/Dropbox/openclaw-backup/agents/<agent-id>/` — create the directory and populate `SOUL.md`, `IDENTITY.md`, `USER.md`, `AGENTS.md`, and `MEMORY.md` from the `.example` templates in `agents/<agent-id>/`. The three operational docs (`TOOLS.md`, `HEARTBEAT.md`, `CRONS.md`) plus the `manifest.json` stay repo-side at `agents/<agent-id>/`. `python3 agents/shared/deploy.py <agent-id> --bootstrap-configs` scaffolds every `.example` sibling in one pass (plus `probation.md` for fix-it). Edit every file with your real values and delete the `CLAWFORD_BOOTSTRAP_UNEDITED` sentinel from any `.md` that still carries it. Brain-side files are Dropbox-synced to the VPS; repo-side files are gitignored where they contain PII and operator-local otherwise. [Ch 03](03-before-you-start.md) has the template-pattern discipline.
...
**Recommended order:** Mr Fixit → Lowly Worm → Mistress Mouse → Sergeant Murphy → Huckle Cat.

The rationale goes low-risk to high-risk along two independent axes: *infrastructure dependencies* and *blast radius if something breaks*. The per-agent subchapter numbering matches this recommended order.

| # | Agent | Why this order |
|---|---|---|
| 1 | 🦊🔧 **Mr Fixit** | The infrastructure fox. Monitors everything downstream. Deploy him first so every subsequent agent inherits a working health probe — and because the first deploy of any new agent is a minefield, and you want to eat those failures on the infra fox, not on the agent wired to your calendar or your inbox. |
...
You don't have to follow this order. You do have to accept the tradeoffs if you don't. Deploying Huckle Cat first means a relationship agent with no relationships to reason about; the mining pipeline can backfill, but the morning nudges get thin without cross-agent context.
...
3. **What makes this agent hard.** The one specific technical challenge that defines the agent — calendar reconciliation, email mining, transcript parsing, or similar. One paragraph.
