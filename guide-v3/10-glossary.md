...
*Guide v3 · net-new in v3 · last revised Phase 7d · (renumbered from Ch 11 after Ch 10 CLI reference was retired)*
...
**Agent.** One of the Python + prompt packages under `agents/{agent-id}/`. In a default Clawford fleet, there are six: [Mr Fixit](07-1-mr-fixit.md), [Lowly Worm](07-2a-lowly-worm-newsfeed.md), [Mistress Mouse](07-3-mistress-mouse.md), [Sergeant Murphy](07-4-sergeant-murphy.md), [Huckle Cat](07-5-huckle-cat.md), and [Hilda Hippo](07-6-hilda-hippo.md). Each has its own workspace under `~/.clawford/{agent-id}-workspace/`, its own cron surface, and its own identity files.

**Baileys.** The reverse-engineered whatsapp-web library used by [Mistress Mouse](07-3-mistress-mouse.md#the-whatsapp-chapter) and (on a separate account) by [Huckle Cat](07-5-huckle-cat.md) for reading WhatsApp chat history. Comes with a ban risk on personal numbers; see the chapter.
...
**`chattr +i`.** Linux "immutable" filesystem attribute. Applied to `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `AGENTS.md` in every agent workspace after every deploy. Blocks writes even by root, which is the whole point — an LLM code path that tries to rewrite its own identity file fails with `Operation not permitted`. See [Ch 08 § Defense layer 1](08-security-and-hardening.md#defense-layer-1--os-level-immutability).

**Clawford.** The fleet runtime. A personal single-operator AI agent system built on plain Python, host crontab, `codex` via ChatGPT Plus, Dropbox, and Telegram. No gateway container, no platform SDK, no custom CLI. See [Ch 01 — What is Clawford?](01-what-is-clawford.md) for the introduction and [Ch 02 — What Clawford isn't](02-what-clawford-isnt.md) for the manifesto.
...
**Drift detection.** Safeguard 4 in `deploy.py`. Refuses to deploy if the workspace on the VPS has changed since the last deploy — i.e., if somebody edited a live workspace file by hand. Blocking, not advisory. See [Ch 08 § Defense layer 3](08-security-and-hardening.md#defense-layer-3--the-deploy-tool-safeguards).

**`.env`.** Per-agent workspace-local environment file. Gitignored. Contains bearer tokens, proxy credentials, TOTP secrets, Telegram bot tokens. Loaded by `agents.shared.env.load_env` on script startup. See [Ch 09 § .env schema](09-scripts-and-configs.md#env-schema).
...
**Liberation.** The project of migrating the fleet off the pre-Clawford gateway platform ("OpenClaw") onto a Clawford-native stack. Phases 0 through 7 (approximately 2026-04-02 to 2026-04-15). Phase 7 completed on 2026-04-15 with the `.openclaw/` → `.clawford/` rename and the guide-v3 migration.

**Local-then-SCP.** The auth-flow idiom from [Ch 07-7 Idiom 1](07-7-auth-architectures.md#the-three-cross-cutting-idioms): any auth flow that needs a browser runs on the operator's local machine, produces a token file, and gets SCPed to the VPS after the fact. The VPS never runs a browser.

**Manifest (`manifest.json`).** Per-agent manifest at `agents/{agent}/manifest.json`. Declares `agent_id`, `workspace`, `config_files`, `crons`, and `smoke_test`. Consumed by `deploy.py` at Safeguard 7. See [Ch 09 § manifest.json schema](09-scripts-and-configs.md#manifestjson-schema).

**MCP (Model Context Protocol).** The protocol used by the meeting-transcript vendor integration in [Sergeant Murphy](07-4-sergeant-murphy.md#the-mcp-transcription-integration). Currently the only MCP consumer in a default fleet. Uses OAuth 2.1 for consent with the same local-then-SCP flow as Google OAuth.

**Mining pipeline.** The one-time, run-locally, human-review-gated seed for [Huckle Cat's](07-5-huckle-cat.md#the-mining-pipeline) people directory. Seven miners (Gmail, Google Calendar, Google Contacts, MCP transcription provider, WhatsApp, Google Messages, Workflowy) + aggregator + LLM enrichment + review markdown + `--finalize`. Step 0 of a Huckle Cat deploy, not optional.
...
**People file.** A per-person markdown file in the shared brain under `people/{email-slug}.md`. Contains name, email, phone, circle (family/work/friend/acquaintance), relationship_type, tone, context_notes, last_interaction, upcoming_interactions, and an array of durable facts. Written by [Huckle Cat's](07-5-huckle-cat.md) mining pipeline + maintained by `daily-refresh.py`.

**Persistent profile.** A browser profile directory that survives across Playwright or Camoufox sessions, preserving cookies + local storage so that an authenticated session doesn't need to re-login on every script run. Used by [Lowly Worm social](07-2b-lowly-worm-social.md) (Playwright + LinkedIn), [Mistress Mouse](07-3-mistress-mouse.md) (Baileys), [Huckle Cat](07-5-huckle-cat.md) (Google Messages), and [Hilda Hippo](07-6-hilda-hippo.md) (Costco + Amazon).
...
**Safeguard.** A deterministic Python check in `deploy.py` that either passes or fails, with no gray area. Nine active safeguards (1 Backup, 2 Source-cleanliness, 3 UPDATE confirm, 4 Drift detection, 5 Workflow banner, 6 Smoke test, 7 Manifest semantics, 9 Cron message discipline, 10 Config source classification). Two retired (8 exec-approvals baseline, 11 docker-compose.yml drift). See [Ch 08 § Defense layer 3](08-security-and-hardening.md#defense-layer-3--the-deploy-tool-safeguards).

**Script contract.** The three-rule contract every cron-invoked script in the fleet obeys: (1) always exit 0, (2) emit exactly one JSON line on stdout, (3) never execute shell. Enforced by `contract_wrap.py` at the import surface and by Safeguard 9 at the deploy boundary. See [Ch 08 § Defense layer 2](08-security-and-hardening.md#defense-layer-2--the-script-contract).

**Shared brain.** See [Brain](#a%E2%80%93z).

**Shared library.** `agents/shared/*` — the world-access layer, organized by Tier 1 (clean APIs), Tier 2 (stock Playwright), Tier 3 (hardened Camoufox), plus a handful of ops modules. See [Ch 06 — Infra setup](06-infra-setup.md) and [Ch 09 Category 1](09-scripts-and-configs.md#category-1--shared-library).
...
**Soft constraint.** A rule encoded as SOUL.md text (e.g., "never modify this file"). Useful as documentation for humans; **not** a security mechanism. An LLM given a direct instruction will happily violate a soft constraint. Hard constraints (OS-level file permissions + `chattr +i`) are the security layer. See [Ch 08 § The soft-constraint trap](08-security-and-hardening.md#defense-layer-1--os-level-immutability).
...
**Workflowy routing rule.** The bright-line rule from [Ch 07-3 § The routing boundary](07-3-mistress-mouse.md#the-routing-boundary-with-sergeant-murphy): if a calendar event has a corresponding Workflowy item, it is a meeting and [Sergeant Murphy](07-4-sergeant-murphy.md) owns it; if not, it is an event and [Mistress Mouse](07-3-mistress-mouse.md) owns it. Both agents check Workflowy presence before acting on any event.
...
**The 5x resend incident (2026-04-14).** [Sergeant Murphy](07-4-sergeant-murphy.md#the-5x-resend-incident). Post-meeting-scan cron used cache files as a delivery queue, re-sent the same debrief five times over 2.5 hours until the operator pulled the cron by hand. Rule: [cache-is-not-a-queue](#a%E2%80%93z). Silent prior on 2026-04-08 discovered during post-mortem.

**The Costco saga (April 5–12, 2026).** [Hilda Hippo](07-6-hilda-hippo.md). Four-act narrative covering the cold-start collapse (April 5–9, persistent daemon pivot), the B2C public-client discovery (April 12), the 2010 setCookie bug (April 12), and the Act IV stabilization (April 12–15). Rule: auto-MFA is non-optional for Shape 5 vendors.

**The probation episode (2026-04-11).** [Mr Fixit](07-1-mr-fixit.md#the-probation-episode). `chr()` approval incident that violated three agent rules and triggered a brain transplant (commit `945ee28`). Produced the five Diagnostic Discipline rules, the P1-P4 failure framework, and the pre-baked `retire.sh`.

**The smart-reply chip incident (2026-04-14).** [Lowly Worm social](07-2b-lowly-worm-social.md#the-smart-reply-chip-incident-2026-04-14). DM enrichment path accidentally triggered LinkedIn's smart-reply chip, auto-replying five times to a dormant thread. Rule: direct URL navigation only, no clicks in the enrichment loop, network-level send-block via `page.route`.

**The stale-dates bug (2026-04-14).** [Huckle Cat](07-5-huckle-cat.md#the-stale-dates-bug-and-the-daily-refresh-fix). Morning relationship nudge flagged contacts the operator had just texted because the mining pipeline stamped `last_interaction` once at deploy time and never refreshed. Fix: `daily-refresh` cron re-mines a 14-day window every morning. Second-order: 6-attendee cap on transcript-based refresh to prevent all-hands meetings from silently clearing the overdue list.

## Retired terms

Terms that used to matter in the pre-liberation fleet and are now historical. Flagged here so readers of old commit messages, older documentation, or frozen [guide-v2](../guide-v2/index.md) content can find what the term means without assuming it refers to live infrastructure.

**Exec approvals / exec-approvals allowlist.** Retired. The pre-liberation mechanism for gating what commands an agent was allowed to run. Safeguard 8 (which enforced the allowlist baseline) retired in Phase 7a. References in live code are bugs.

**OpenClaw.** Retired. The pre-liberation gateway container that sat between the agents and the world. Removed entirely during Phase 6.5 + Phase 7a of the Clawford liberation. References in historical context (commit messages, retired-file tombstones) are fine; references in live code are bugs.
...
**`~/.openclaw/`.** Retired directory path. Renamed to `~/.clawford/` in Phase 7b (2026-04-15). References in live code are bugs; the regression guard at `agents/shared/tests/test_phase7b_openclaw_paths_retired.py` prevents them from recurring.

**`~/openclaw/` (no dot).** Retired directory path. The pre-liberation VPS env-file location. Deleted in Phase 7b; `.env` now lives at `~/clawford/.env`.

**Safeguard 8 (exec-approvals baseline).** Retired in Phase 7a. Enforced drift detection against a platform-level exec-approvals baseline that no longer exists. Tombstoned in `deploy.py` source comments.

**Safeguard 11 (docker-compose.yml drift).** Retired in Phase 7. Enforced drift detection against a `docker-compose.yml` that no longer exists (gateway container was removed). Tombstoned in `deploy.py` source comments.

## See also

- [Ch 01 — What is Clawford?](01-what-is-clawford.md) — the introduction
- [Ch 02 — What Clawford isn't](02-what-clawford-isnt.md) — the manifesto
...
- [Ch 08 — Security and hardening](08-security-and-hardening.md) — the three defense layers
- [Ch 09 — Scripts and configs](09-scripts-and-configs.md) — the file-level reference
