...
*Last updated: 2026-04-20 · Reading time: ~20 min · Difficulty: hard*
...
2. **Proposed reverting a commit** from the same day without checking `git log` to understand why it was committed. The commit he wanted to revert was unrelated to the approval; he had seen it in the recent history and plausibly assumed causation.
...
Later the same day (2026-04-11, 22:06 UTC) came what I called at the time a **brain transplant** — not a model swap but a tooling + discipline rework. Five pieces landed in one commit:
...
| `propose_refresh_session(source)` | Forces a session refresh for the named source. `source="google"` calls `google_oauth.refresh_if_stale(token, max_age_days=0)` against every workspace's `token.json` that claims a Google scope. Extensible: new sources register their own refresh helper and the tool dispatches on `source`. | `agents/shared/google_oauth.py`. | 2h |

The pattern is the stage-then-confirm shape every producer tool in the fleet follows. Producer tools (`propose_*`) appear in the LLM tool manifest; their executors stage a pending action and return a `__pending_action__` marker. The dispatcher sees the marker, attaches a Telegram inline keyboard with `[Confirm] [Cancel]` buttons, and the operator's tap routes to a callback-only `confirm_*` executor that runs the actual side-effect. The confirm executors are deliberately not exposed in the manifest — the LLM cannot invoke them directly, only stage actions for the operator to approve.
...
- [Ch 08 — Your first agent](08-your-first-agent.md) — the general seven-step deploy arc Mr Fixit inherits from.
- [Ch 19 — Security and hardening](19-security-and-hardening.md) — the `chattr +i` immutable-identity story and the defense-in-depth posture.
- [The Ballad of Mr Fixit](ballad-of-mr-fixit.md) — the five-act tragedy. Required reading at least once. Ideally before you deploy the fox rather than after.
