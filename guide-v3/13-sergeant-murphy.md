...
*Last updated: 2026-04-23 · Reading time: ~35 min · Difficulty: hard*
...
2. Any attendee's domain — **or the event organizer's domain** — matches the shared `RECRUITER_DOMAINS` set (ATS + retained-search platforms) → `recruiter-screen`. Same domain list Huckle's detector uses; single source of truth at `agents/shared/recruiter_domains.py` so the two agents agree. The organizer check catches in-house scheduling systems like `schedule@interview.adobe.com` / `no-reply@recruiting.amazon.com`: the interviewer's name lives only in the description body, so the invite carries no useful attendee — without the organizer signal, the meeting would fall through to `general`. Big-company ATS subdomains (`interview.*`, `schedule.*`, `recruiting.*`, `talent.*`, `careers.*`, `hire.*`, `hiring.*` on any 3+ label domain) match without needing per-company enumeration.
...
### Company research — the other half of the prep

The 8-field schema assumes the LLM already knows something concrete about the company. Out of the box, it does not. A recruiter screen at Anthropic and a recruiter screen at Acme Widgets produce identically generic `evaluation_questions` if the prompt has nothing external to ground them in. *"Tell me about the team structure"* is the tell — any meeting prep whose questions could be recycled for any other company is a meeting prep that didn't earn its LLM call.

The fleet-shared `agents/shared/company_research.py` closes the gap. Before the prep LLM call fires, `resolve_company_name_for_prep` picks a company name in this order: (1) a `target_company` match on an attendee domain; (2) `_extract_company_token(event)` parsing titles like *"Interview with Coinbase"*; (3) attendee-domain inference, skipping LinkedIn relays; (4) organizer-email inference, handling ATS subdomains like `schedule@interview.adobe.com` → Adobe. When it returns a name, `research_company(name, operator_context=...)` returns a `CompanyBrief` carrying `what_they_do`, `stage_signal`, recent news bullets, role context from JD signals, and an `operator_fit` block with `strength_angles`, `concerns`, `questions_to_ask`, and `hooks_to_drop`. The full spec lives in [§ External company research in Ch 14](14-huckle-cat.md#external-company-research-grounding-the-pitch); the point here is that Murphy consumes it as a **COMPANY BRIEF** prompt block between SELF CONTEXT and TARGET COMPANY MATCH, and the LLM is told explicitly to ground `fit_pitch`, `compelling_angle`, and `evaluation_questions` in those specifics rather than fabricating generic alternatives.

The cache is shared with Huckle at `~/.clawford/company-research-cache/`. If Huckle's cold-recruiter compose already researched Anthropic earlier today, Murphy's prep run is a zero-cost cache hit. One LLM spend, both agents benefit.

Two calibration scars landed on the day the layer shipped, both from simulating tomorrow's actual recruiter interviews:

- **The LinkedIn relay leak.** First prep run on a self-booked *"Interview with Coinbase"* event (the Abby-via-LinkedIn-DM shape) came back researched as *"LinkedIn"* — the Gmail-recruiter-lookup had resolved Abby to `hit-reply@linkedin.com`, and naive domain inference turned that into a company. Fix: LinkedIn domains are in the same no-signal set as third-party ATS platforms, and the resolver prefers `_extract_company_token(event)` over attendee-domain inference. *"Interview with Coinbase"* → `Coinbase`, every time.
- **The empty-attendee organizer.** Adobe Leadership Chat invites ship with title *"Meeting Confirmation - Sam Smith"*, zero attendees populated, and the only company signal buried in the organizer (`schedule@interview.adobe.com`). First pass returned no brief at all — the resolver never looked at the organizer. Fix: step 4 walks the organizer email, and `_infer_company_from_email` handles ATS subdomains plus personal-email providers (so self-booked invites whose organizer is the operator's own Gmail don't false-fire on *"Gmail"*).

Both scars share a shape worth naming: **external signals can live in any field of a calendar event, and each field has its own failure mode.** A resolver that only checks attendees works until it meets an ATS-scheduled invite; a resolver that only checks the title works until it meets a self-booked meeting with a generic confirmation title. The current resolver walks four fields in order because the first three fail for at least one common invite shape — and the fourth (organizer) brings its own trap (personal-email providers) that the no-signal set absorbs. Each rung earns its place by reference to a real invite that otherwise fell through.
...
The Adobe / Amazon-style ATS invites that forced the rule-2 organizer extension also forced a title-derivation fallback. With no attendees and no `target_company` entry, the original inline derivation produced `Unknown #JobSearch` — accurate but useless. `derive_meeting_title(prep)` now walks a precedence chain before resorting to `Unknown`: (1) first non-empty `attendees[].name`; (2) first `Interviewer(s):` / `Host(s):` / `With:` / `Meeting with:` line in the description body (trailing parentheticals trimmed, comma-lists reduced to the first name); (3) `Unknown`. Company tag similarly prefers the operator-curated `self_context.target_company`, then the organizer's ATS subdomain stem via `extract_company_from_ats_domain` in `recruiter_domains`. Third-party ATS platforms (greenhouse, lever, `hire.withgoogle.com`) deliberately yield no tag — the host domain isn't the interviewing company, and a wrong tag is worse than a missing one. The first call that tripped this path landed as `Alyssa Bonefas #JobSearch #Adobe` from an invite whose summary was literally `"Meeting Confirmation - Sam Smith"`.
