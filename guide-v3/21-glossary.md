...
**Agent.** One of the Python + prompt packages under `agents/{agent-id}/`. In a default Clawford fleet, there are six: [Mr Fixit](09-mr-fixit.md), [Lowly Worm](10-lowly-worm-newsfeed.md), [Mistress Mouse](12-mistress-mouse.md), [Sergeant Murphy](13-sergeant-murphy.md), [Huckle Cat](14-huckle-cat.md), and [Hilda Hippo](15-hilda-hippo.md). Each has its own workspace under `~/.clawford/{agent-id}-workspace/`, its own cron surface, and its own identity files.

**Baileys.** The reverse-engineered whatsapp-web library used by [Mistress Mouse](12-mistress-mouse.md#the-whatsapp-chapter) and (on a separate account) by [Huckle Cat](14-huckle-cat.md) for reading WhatsApp chat history. Comes with a ban risk on personal numbers; see the chapter.
...
**Residential proxy.** A proxy service that routes traffic through residential IP addresses rather than datacenter IPs, to avoid vendor-side bot detection. Used by [Hilda Hippo](15-hilda-hippo.md) for Costco and Amazon. Sticky-port variant (port 10000 in the fleet's setup) holds the same residential IP for the duration of a session.

**Read tool.** A tool in the `TOOLS` manifest that has no side effects — it reads cached state and returns structured data for the LLM to use in composing a response. Examples: `get_fleet_health` (Mr Fixit), `get_events_for_day` (Mistress Mouse), `get_delivery_digest` (Hilda Hippo). See [Ch 18 § The tools.py pattern](18-the-inbox.md#the-toolspy-pattern).
...
**TOTP.** Time-based One-Time Password. The 6-digit 2FA code generated from a seed secret, used by [Hilda Hippo's](15-hilda-hippo.md) auto-MFA path on Amazon. Seed lives in `.env` as `{VENDOR}_TOTP_SECRET`, computed by `pyotp` at login time.
