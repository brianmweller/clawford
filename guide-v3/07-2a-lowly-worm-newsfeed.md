...
- He was the **Phase 3 pilot** for the Clawford liberation — the first agent to migrate off the old LLM cron runtime onto the post-liberation stack (`agents.shared.llm.infer`, host crontab, the compose/deliver split). Every pattern he shipped in that migration became the template the other five agents inherited. The scar tissue is in the deployment walkthrough section.
- If you *also* want Lowly to watch a social channel (LinkedIn, X, Bluesky, etc.) for filtered notifications and consolidated DMs, that's an optional extension covered in [Ch 07-2b](07-2b-lowly-worm-social.md). **The core newsfeed stands on its own and is useful without any of that.** Deploy this chapter first; add 07-2b only if you care about the social half.
...
| `linkedin-keepalive` | `0 */6 * * *` | Pure Python | Runs `linkedin-keepalive.py`, which launches a headless Playwright browser against the persistent LinkedIn profile every six hours to keep the session warm. Only relevant if you deploy [Ch 07-2b](07-2b-lowly-worm-social.md); leave it unregistered otherwise. |

Plus the fleet-wide `morning-fleet-deliver-host.sh` from [Ch 07-1 Mr Fixit](07-1-mr-fixit.md), which at `0 12 * * *` reads `cache/morning-items.json`, chunks it into per-item Telegram messages with inline 👍/👎/📖 buttons, and posts them via `NEWSDIGEST_BOT_TOKEN`.
...
- [Ch 07-2b — Lowly Worm: social channel extension](07-2b-lowly-worm-social.md) — the optional extension that adds LinkedIn (or any other social channel) notifications and DM consolidation on top of the core newsfeed. Read only if you care about the social half.
- [Ch 07-0 — Your first agent](07-0-your-first-agent.md) — the general seven-step deploy arc Lowly inherits from.
- [Ch 07-1 — Mr Fixit](07-1-mr-fixit.md) — the host-cron install pattern, the fleet-health story, and the `morning-fleet-deliver-host.sh` cron that handles Lowly's delivery.
