...
- **`agents/shared/playwright_profile.py`** — `launch_persistent_profile(profile_dir, *, headless, xvfb_display)`, `ensure_xvfb(display_num)`, `cleanup_profile_lock(profile_dir)`. The mechanics of "launch Chromium in a way that preserves the logged-in session across reboots." The canonical Tier 2 consumer is the Google Messages Web scraper.
...
The parts of the outside world that fingerprint you, detect headless browsers, present interactive Azure B2C login flows, and aggressively rotate their anti-bot rules. Any retail site with a fraud team lives here. Stock Playwright fails in this layer — not because it *can't* log in, but because it gets flagged two sessions later and starts serving interstitials.
...
- **`agents/shared/retry_policy.py`** — `classify_selfasserted_response`, `should_retry`, exponential backoff with the tuning a hostile-vendor login flow needs. Promoted out of a single agent's local copy when a second Tier 3 consumer came along.
...
A Tier 3 integration against a hostile vendor has a shape that emerges whether you like it or not. It took one agent two weeks of evenings to find, and the patterns below are what survived.

**Chase the JWT for a public client before you build the browser dance.** When a vendor's auth runs on Azure AD B2C (or any OIDC provider with custom policies), the id_token it hands you often lists *multiple* client ids in the `userIdentities` claim — the confidential web client you logged in through, plus one or more additional clients the mobile app uses. The mobile client is often PKCE-capable and accepts `grant_type=refresh_token` with no client_secret. If you can find that second id by decoding a JWT you've already captured, you can collapse a 45-90 second Camoufox flow into a ~1 second `curl_cffi` HTTPS call. One probe script, one successful exchange, and you're done with the browser for 99% of runs. The browser step becomes a fallback, not the hot path. Try this *before* you invest a week in Chrome-DevTools-based auth archaeology.

**Keep the browser warm.** Cold-starting Camoufox costs 30-45 seconds per run for profile load, anti-detect init, and page warmup. If a cron needs to refresh a token every N minutes and the refresh takes 60 seconds, a meaningful fraction of your fleet budget goes to orchestration. The fix is a long-running supervisor process — started once at host boot via systemd — that keeps one Camoufox instance alive and performs the refresh against an already-warm SSO session. A second worker hits the vendor's front-door URL every 12 hours purely so the Akamai sensor script inside the profile stays fresh. Cold start: ~60s. Warm refresh: ~5s. Do not pay 60 seconds per cron tick forever.

**The cheapest residential proxy is your home ISP over Tailscale.** Datacenter IPs (Hetzner, DigitalOcean, any hoster's AS) get 403'd on hostile-vendor auth endpoints within minutes. Paid residential pools (DataImpulse, IPRoyal, Bright Data) work — for a while. Akamai's reputation engine decays paid residential IPs in hours once it fingerprints them as automation, and the fix turns into 100+ engineering hours plus $50-100/month of premium residential traffic in an arms race you will not win. The move that collapses the whole adversarial game: an `autossh` systemd service on the VPS that holds a persistent SSH tunnel over Tailscale to a machine on your home network, exposing `socks5://127.0.0.1:1080` on the VPS. Linux's `ssh -D` makes this a one-liner. Traffic exits your home router, Akamai sees an IP it has been watching do ordinary household things for years, and adding one script-driven reauth every few days is noise-level. The tunnel's health probe is a SOCKS5 CONNECT to a known endpoint (`1.1.1.1:443` works) — a local `netstat` saying the listener is up tells you nothing about whether the remote peer is awake. If the machine at the other end sleeps, the probe fails cleanly and the daemon defers the refresh without incrementing any backoff counter. Tunnel-offline is not reputation decay.

**Automate the MFA or don't ship the feature.** Every Camoufox launch is "a new device" from a hostile vendor's perspective. Every new device triggers MFA. If you don't automate the MFA prompt, every overnight cron fails on the code-entry modal and you wake up to alerts. The working pattern: store the TOTP base32 seed in a separate `secrets.env` file (chmod 600, gitignored, sourced as a second env_file in the host wrapper), and use `pyotp` to generate the six-digit code at browser launch time. The split from `.env` is load-bearing — a regular `.env` gets dumped into paste buffers, screen-shared into Zoom, copied into Slack DMs when you're asking someone for help; a leaked `.env` with operational passwords is recoverable, a leaked `secrets.env` with a TOTP seed is account takeover (TOTP seeds don't get invalidated by a password reset). Never ask the operator to re-auth manually. The memory rule is *"browser auth is expensive"*: build auto-recovery or don't build the feature at all.

**A fallback ladder needs per-step backoff, not a global retry loop.** The structural shape that actually survives a Tier 3 vendor under stress: Step 0 is the fast path (refresh_token grant via `curl_cffi`, direct from VPS, no browser). Step 1 is silent browser refresh against the saved SSO cookie, routed through the home-ISP tunnel. Step 2 is a real credential challenge, same route. Each step has its own exponential-backoff counter in `cache/refresh-backoff.json` — one failure skips the step's next 15 minutes, two skips 30, three skips 60, four-plus skips 120 and fires one Telegram nudge. Success resets the counter. The critical property: **a naive `*/5` retry loop against a rate-limited anti-bot service feeds more signal into the vendor's heuristics on every tick, which makes the next tick worse.** The daemon poisons its own reputation faster than the vendor could have on its own. Exponential backoff per step lets the reputation decay instead of amplify. Tunnel-offline / provider-outage conditions skip the step without incrementing backoff (they're not signal decay). The ladder gets quieter under failure, not louder.

**`auth_time` is the only observable signal that a real reauth ran.** The subtle failure mode: a function called `try_full_reauth` that injects the existing SSO cookie into the browser context and navigates the login form *completes via SSO* — no credential challenge, no password submission, no new authentication event. The id_token returned looks fresh because it has a new `iat`/`exp`, but the session's absolute-cap clock (~72 hours on Azure B2C's `Grant Issued Time`) did not move. The function was cosmetic for the entire life of the deployment. The fix: decode the new id_token's `auth_time` claim and refuse to advance the reauth scheduler unless `now - auth_time ≤ 300s`. The refresh_token is JWE (opaque), the id_token's `exp - iat` lies (vendors issue fixed-duration tokens regardless of freshness), and `auth_time` — the seconds-since-epoch of the last real `/authorize` event — is the only field a client can actually trust. Verify it, or your "scheduled reauth" is a placebo.

**The cron-entry path is not the same code as `--step X`.** `python3 daemon.py --step residential` exercises one step in isolation. `python3 daemon.py` (no args) runs `main()`, which does step selection, env-var mutation, health gating, backoff reads, failure-mode routing. A refactor that works via `--step` can still be broken via `main()`. The only real test of "is this cron going to succeed tonight" is firing the exact invocation the cron does, in the cron's environment. And when you spawn a subprocess whose `__main__` wraps every failure in a `{"status":"error"}` envelope and exits 0 per the script contract — and you then check `result.returncode == 0` — every failed step looks like success from the parent's perspective. Parse the envelope's `status` field from stdout's last non-empty line. The returncode is a tautology by design.

None of this is mandatory for a Tier 1 or Tier 2 integration. All of it is load-bearing the moment you step into a vendor's fraud perimeter. If any of the patterns above sound like overkill for what you're about to build, the target is probably not Tier 3 and you're in the wrong place in this document.
