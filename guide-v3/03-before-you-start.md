...
*Last updated: 2026-04-20 · Reading time: ~15 min · Difficulty: easy*
...
- Budget **~$50/month** to run Clawford (VPS + ChatGPT Plus subscription). Budget **2-3 days** to your first working agent and **2-4 weeks of evenings** to a full fleet. Don't lowball either number.
- Run Clawford on a dedicated VPS, not your daily driver. Deploy Mr Fixit first. Use Telegram for the operator-agent channel. Those are the cheapest "listen to me" decisions in the whole guide.
...
**Optional — if you build a Tier 3 integration:**

- A **residential proxy subscription** (IPRoyal, Bright Data, or similar). Only matters if you deploy a Tier 3 integration against a vendor with active bot defense (retailers, fraud-sensitive portals) — datacenter IPs get blocked fast on those. The public fleet in this repo has no such integration; buy this only when the specific workload needs it. Budget **~$1/month** for small-HTTPS-call workloads; full-page scrapes through the proxy on every query land in the $8-30/month range. A better answer, when it's available, is a SOCKS5 tunnel to your home network — see [Ch 06 — Tier 3 in practice](06-infra-setup.md#tier-3-in-practice).
...
- **Ongoing operations after the fleet is stable:** a few hours per week of "something broke and I need to look at it," front-loaded after each new deploy and whenever an upstream service (Google, a news source, a messaging API) changes something under you.
...
Telegram is the operator-agent channel. The official Bot API has no ban risk, allows unlimited proactive messages, and each bot gets its own name and avatar so you can tell which agent is talking to you at a glance. The public fleet in this repo wires every agent to its own Telegram bot.

(Other messaging platforms that look tempting — WhatsApp via reverse-engineered linked-device clients, iMessage via third-party relays — all sit on the wrong side of vendor terms and carry account-ban risk. If you absolutely need one for a specific delivery target, treat it as a *hand-off point* the operator forwards to manually, never as the agent's primary channel.)
...
| **Cost** | API-key-only billing, free Dropbox tier, skip any Tier 3 proxy | Surprise bills are possible; any Tier 3 integration needs residential egress |
...
| **Breadth of agents** | Deploy all five, integrate broadly | Each additional agent is its own rabbit hole |
| **Speed to first win** | Deploy Mr Fixit only | A smaller surface until you add the next agent — a single-agent setup doesn't even need the shared brain, so it's lean rather than crippled |
...
> 🧨 **Pitfall.** Picking a messaging platform "just to try it" because your family already uses it. **Why:** reverse-engineered linked-device clients (WhatsApp-via-Baileys being the canonical example) get personal numbers banned on a non-deterministic schedule, and most consumer messaging platforms have a 24-hour inactivity window that silently drops the exact overnight alerts you'd want most. **How to avoid:** stand the fleet up on Telegram first, always. If you later need another channel for human reachability, wire it as a *delivery target* the operator hands off to manually, not as the agent's primary channel.
