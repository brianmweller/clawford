...
*Last updated: 2026-04-23 · Reading time: ~60 min · Difficulty: hard*
...
### External company research — grounding the pitch

The SELF CONTEXT and search-status layers tell the LLM a lot about the operator. They tell it nothing about the company the recruiter is pitching. Post-calibration, the drafts engaged with the pitch and scheduled the call correctly, but they couldn't name anything specific about the company's current state — no *"the Series D + agent-SDK push"* hook, no *"how does X map to Y's launch?"* question. The FIT CHECK and voice layers were doing their jobs; the residual gap was between what the operator knows about themselves and what the recruiter implicitly assumes the operator will look up before the call.

A fleet-shared enrichment pass closes that gap at compose time. Three parts:

1. **Extract.** `agents/shared/recruiter_extract_lib.py` runs a small Codex call on the inbound and returns `{company_name, role_title, company_domain}`. A mechanical domain-inference prior handles the easy case — an email from `@anthropic.com` obviously means Anthropic — so the LLM confirms rather than reasoning from zero. LinkedIn InMail relays (`hit-reply@linkedin.com`), third-party ATS platforms (`jane@lever.co`), and personal-email providers (`gmail.com`, `icloud.com`) are in an explicit no-signal set: the sender domain encodes nothing about the hiring company, and the body is the only source worth asking about.

2. **Research.** `agents/shared/company_research.py::research_company(name, role_title=..., operator_context=...)` fires two Brave Search queries — *"X what they do"* and *"X news"* — plus a third targeting the JD if `role_title` is present, then runs one Codex synthesis pass that turns the snippets into a `CompanyBrief`: `what_they_do`, `stage_signal`, `recent_news` (dated bullets), `tech_or_product_hints`, `role_context` from the JD signal, and an `operator_fit` block with `strength_angles` (which of the operator's strength themes map to this company's stated problems), `concerns` (honest level/stage/domain mismatches — the operator prefers filter over flattery), `questions_to_ask`, and `hooks_to_drop` (one-or-two phrases that prove the operator read about them). Confidence is marked `low` when the snippets are vague so downstream readers can calibrate.

3. **Inject.** The compose prompt gains a **COMPANY CONTEXT** block between SELF CONTEXT and FIT CHECK, positioned so `hooks_to_drop` stay fresh in the LLM's attention when it reaches STRATEGY. The hooks and questions aren't mandatory — the model may refine or replace them — but they're the floor, not something the model has to invent from zero.

Cache lives at `~/.clawford/company-research-cache/{slug}.json` with a 30-day TTL, keyed by normalised slug so *"Anthropic, PBC"* and *"Anthropic"* collapse to the same entry. The cache is fleet-shared: Sergeant Murphy's 3:30 AM PT prep for an Anthropic hiring-manager seeds it, and Huckle's 10 AM reply to an Anthropic recruiter later that day is a zero-cost hit. Two agents, one LLM spend — the cache layout is the fleet-thinking discipline made literal.

Degrades silently. Brave failure → empty snippets, synthesis still runs and marks the brief low-confidence. Codex failure on synthesis → the brief is skipped, the draft runs against the pre-enrichment prompt, no cache file is written so the next call retries. Any failure mode is a quality regression, never a pipeline break.
